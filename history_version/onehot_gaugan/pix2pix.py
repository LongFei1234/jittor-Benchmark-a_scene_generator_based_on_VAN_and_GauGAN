...
eval_trans_real = transform.Compose([
    transform.ImageNormalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
])

dataloader = ImageDataset(opt.data_path, mode="train", trans_real=trans_real, trans_seg=trans_seg).set_attrs(
    batch_size=opt.batch_size,
    shuffle=True,
    num_workers=opt.n_cpu,
)
val_dataloader = ImageDataset(opt.data_path, mode="val", trans_real=trans_real, trans_seg=trans_seg).set_attrs(
    batch_size=opt.batch_size,
    shuffle=True,
    num_workers=1,
)
...
def eval(epoch, writer):
    ...
    for i, (_, real_A, photo_id) in enumerate(val_dataloader):
        fake_B = generator(jt.randn(real_A.shape[0], 256), get_one_hot(real_A))
        if i == 0:
            # visual image result
            img_sample = np.concatenate([eval_trans_real(real_A).data, fake_B.data], -2)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (real_B, real_A, _) in enumerate(dataloader):
        ...
        loss_G = (loss_G_GAN + lambda_pixel * loss_G_Feat + lambda_vgg * loss_vgg + lambda_L1 * loss_G_L1 + KLD_loss) / 5

        optimizer_G.backward(loss_G)
        optimizer_G.step()
        optimizer_E.backward(loss_G) 
        optimizer_E.step()
